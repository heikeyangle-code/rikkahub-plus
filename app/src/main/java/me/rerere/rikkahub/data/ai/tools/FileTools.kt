    ...
    return listOf(
        Tool(
            ...
            description = buildString {
                ...
                appendLine()
                appendLine("file_read behavior:")
                appendLine("- Reads file contents with optional offset/limit (pagination).")
                appendLine("- Directories are listed automatically if path is a directory.")
                appendLine("- Can read images (PNG, JPG).")
                appendLine("- Reports if file is unchanged since last read.")
                appendLine()
                appendLine("file_write behavior:")
                appendLine("- Creates new files or overwrites existing ones.")
                appendLine("- For existing files, read first then modify and write.")
                appendLine()
                appendLine("file_list behavior:")
                appendLine("- Lists files and directories with size info.")
                appendLine()
                appendLine("file_search behavior:")
                appendLine("- Two modes: name (by filename glob) and content (search inside files).")
                appendLine("- Supports regex in content mode, glob in name mode.")
                appendLine()
                appendLine("file_copy / file_move behavior:")
                appendLine("- Works for both files and directories.")
                appendLine("- Move falls back to copy+delete across mount points.")
                appendLine()
                appendLine("file_mkdir / file_delete behavior:")

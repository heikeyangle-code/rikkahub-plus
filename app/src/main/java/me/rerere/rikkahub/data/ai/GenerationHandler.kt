    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【塔罗/雷曼】arcanite(Python,78张+36雷诺曼+牌阵+正逆位,洗牌抽牌) | 塔罗9层数据:core_meanings → position_interpretations(7牌位) → question_contexts(爱情/事业/灵性/财务/健康) → elemental_correspondences(元素/星座/灵数/行星/水晶等10项) → symbols → affirmations → journaling_prompts → meditation_focus → card_relationships(增幅/挑战/澄清/同类/对立/学习序列6种) | 雷曼:core/timing/topic_contexts/line_reading/combinations/grand_tableau | 深度:查777表→Kaabalah.buildKaabalisticMapData(JS,全映射) | 雷曼抽法同塔罗,system=lenormand | 无需出生")

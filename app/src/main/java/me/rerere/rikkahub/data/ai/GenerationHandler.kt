    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("    (lunar_python排盘+bazi_china神煞 · 黄历择日luohou · 生肖合婚)")
                appendLine("  紫微斗数 → from routes.紫微斗数路由 import __doc__ as R; print(R)")
                appendLine("    (Iztro JS / ziwei_paipan Python / ZiweiNihai 三引擎)")
                appendLine("  塔罗 → from routes.塔罗路由 import __doc__ as R; print(R)")
                appendLine("    (arcanite Python + TarotKit JS 互补)")
                appendLine("  雷诺曼 → from routes.雷诺曼路由 import __doc__ as R; print(R)")
                appendLine("    (arcanite 36张 + GT + FE引擎调度)")
                appendLine("  西洋占星 → from routes.西洋占星 import __doc__ as R; print(R)")
                appendLine("    (NatalEngine + Caelus 231函数 + Astronomy)")
                appendLine("  印度占星 → from routes.印度占星普通 import __doc__ as R; print(R)")
                appendLine("    深度NodeJhora → from routes.印度占星深度版 import __doc__ as R; print(R)")
                appendLine("  人类图 → from routes.人类图路由 import __doc__ as R; print(R)")
                appendLine("  灵数/卡巴拉 → from routes.灵数卡巴拉数秘 import __doc__ as R; print(R)")
                appendLine("  奇门/六壬 → from routes.奇门三式路由 import __doc__ as R; print(R)")
                appendLine("  六爻/梅花/太玄 → from routes.象数卦象路由 import __doc__ as R; print(R)")

    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("    奇门(奇门遁甲/qimen): {year, month, day, hour?, minute?, feature}")
                appendLine("    大六壬(六壬/liuren): {year, month, day, hour?, minute?, feature}")
                appendLine("    六爻(六爻纳甲/liuyao): {method, seed?, year?, month?, day?, feature?}")
                appendLine("    梅花易数(梅花/meihua): {method, seed?, year?, month?, day?, feature?}")

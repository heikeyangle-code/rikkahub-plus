    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【西洋占星】本命盘→问用户选kerykeion(Python,SwissEphemeris)或HoroscopeJS(JS,Kepler+7宫位制)或都跑 | 深析→stellium | 推运→stellium.returns(备:flatlib) | 合盘→immanuel | 日食月食/升落→pyswisseph(备:Astronomy-JS,VSOP87) | 星盘SVG→render_astrology_svg()")
                ...
                appendLine("⚠️ JS引擎: QimenEngine(奇门) | ZiweiNihai(紫微) | IchingShifa(六爻) | TaixuanLib(太玄) | Lunar(农历/八字) | Astronomy(行星/日月食,VSOP87) | HoroscopeJS(本命盘,Kepler+7宫位制) 均通过 eval_javascript 调用, 探索: Object.keys(EngineName)")

    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("")
                appendLine("  各系统 params: 塔罗{spread,seed,question_type?,kaabalah?,cards?} | 雷诺曼{spread,seed,cards?} | 八字{year,month,day,hour,gender,feature?} 紫微{year,month,day,hour,gender,engine?}")
                appendLine("    现代占星{year,month,day,hour,tz,lat,lon} | 传统占星{year,month,day,hour,tz_offset,lat,lon} | 吠陀{year,month,day,hour,tz,lat?,lon?}")
                appendLine("    人类图{year,month,day,hour,tz,gene_keys?,transits?} | 卡巴拉{year,month,day,word?,feature} | 奇门{year,month,day,hour?,feature} 六爻{method,seed?,year?,month?,day?,feature?}")
                appendLine("")

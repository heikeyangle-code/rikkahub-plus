    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【奇门三式】奇门遁甲→QimenEngine(JS,7局法×4流派+断语) | 大六壬→kinliuren | 太乙神数→kintaiyi | 小六壬→lunar_python取月日时→掌诀推算(大安留连速喜赤口小吉空亡)")
                ...
                appendLine("【印度吠陀】Caelus(JS,nakshatras/dasha/vargas/yogas等) | Python侧全删")
                ...
                appendLine("输入要求：八字/紫微需生日时辰+性别 | 黄历/择日/太岁/节气仅需日期 | 六爻/梅花/太玄/荆诀/塔罗无需出生")

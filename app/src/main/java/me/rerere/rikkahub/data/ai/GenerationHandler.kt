    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("命理/玄学一律走 execute_python。各术数对应的首选库：")
                appendLine("【中华正统】八字/四柱/大运→lunar_python.EightChar | 备选:bazi_china,sxtwl  紫微斗数→ziwei_paipan.by_solar()")
                appendLine("【奇门三式】奇门遁甲→kinqimen(日家/时家/刻家) | 大六壬→kinliuren | 太乙→kintaiyi | 皇极→kinwangji | 小六壬→手算(lunar_python取月日时→掌诀)")
                appendLine("【象数易】太玄筮法→taixuanshifa | 荆诀→jingjue")
                appendLine("【六爻/卦】六爻/周易→ichingshifa(需起卦数) | 梅花易数→meihua_yi(需数字)")
                appendLine("【西洋占星】本命盘/星座→kerykeion | 合盘→immanuel | 深析/中点/格局→stellium | 日返月返→stellium.returns | 天文→pyswisseph | 星盘SVG→render_astrology_svg()")
                appendLine("【印度吠陀】南印/北印盘→jhora | 备选:stellium.visualization.vedic")
                appendLine("【农历天文】黄历/择日/建除/太岁/吉神→cnlunar | 公历农历转换→lunar_python | 二十八宿→lunar_python.get28Mans() | 节气→lunar_python")
                appendLine("【塔罗】韦特78张+雷诺曼36张→arcanite(TarotDeck.load) | 正逆位牌义+牌阵")
                appendLine("输入要求：八字/紫微/占星/吠陀需生日+时辰+性别+经纬度 | 合盘需双人生日 | 黄历/择日/太岁/节气仅需日期 | 六爻/梅花/太玄/荆诀/塔罗无需出生")
                appendLine("完整路由以 executor.py docstring 为准；初次用 dir() 自探索API")
                appendLine("</mingli_routing>")
            },

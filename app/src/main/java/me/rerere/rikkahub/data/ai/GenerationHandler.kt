    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("⚠️ 任何命理/玄学/占卜任务，先确定系统，然后用 Python 在 executor.__doc__ 里定位对应标记，只读那一段。禁止全量打印 executor.__doc__。")
                appendLine("</mingli_must_read>")
                appendLine()
                ...
                appendLine("  八字/四柱/大运 → doc[doc.index('【双库并联】'):doc.index('【奇门三式】')]")
                appendLine("  紫微斗数 → doc[doc.index('紫微斗数'):doc.index('【奇门三式】')]")
                appendLine("  奇门/六壬/小六壬 → doc[doc.index('【奇门三式】'):doc.index('【象数易】')]")
                appendLine("  六爻/梅花/太玄/荆诀 → doc[doc.index('【象数易】'):doc.index('【西洋占星】')]")
                appendLine("  西洋占星 → doc[doc.index('【西洋占星】'):doc.index('【印度/吠陀】')]")
                appendLine("  印度占星普通 → doc[doc.index('【印度/吠陀】'):doc.index('【印度占星深度版')]")
                appendLine("  印度占星深度NodeJhora → doc[doc.index('【印度占星深度版'):doc.index('【人类图')]")
                appendLine("  人类图/基因钥匙 → doc[doc.index('【人类图/Human Design】'):doc.index('【塔罗/雷诺曼')]")
                appendLine("  塔罗 → doc[doc.index('╔════ 塔罗'):doc.index('╚════ 塔罗')+1]")
                appendLine("  雷诺曼 → doc[doc.index('╔════ 雷诺曼'):doc.index('╚════ 雷诺曼')+1]")
                appendLine("  灵数/卡巴拉 → doc[doc.index('【灵数学/卡巴拉/数秘】'):doc.index('  【农历/干支/天文】')]")
                appendLine("  农历/天文/luohou → doc[doc.index('  【农历/干支/天文】'):doc.index('【查询路由】')]")
                appendLine("  查询路由/JS引擎 → doc[doc.index('【查询路由】'):doc.index(chr(10)*3+'#')]")

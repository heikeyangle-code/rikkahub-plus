    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【中华正统】八字/四柱/大运→lunar_python.EightChar(备:bazi_china,sxtwl)  紫微斗数→问用户选Iztro(⭐3841原版JS,权威基准)或ziwei_paipan(Python port)或ZiweiNihai(倪海夏天纪)或多方对照")
                appendLine("【奇门三式】奇门遁甲→问用户选kinqimen(2局法)或QimenEngine(7局法+断语)或都跑对照 | 大六壬→kinliuren(纯Python,1939行,9宗门) | 太乙神数→kintaiyi | 皇极经世→kinwangji | 小六壬→lunar_python取月日时→掌诀推算(大安留连速喜赤口小吉空亡)")
                ...
                appendLine("【西洋占星】JS首选Caelus(29工具:本命/推运/合盘/日食/吠陀/择日,零依赖VSOP87D,与SwissEphemeris对齐) | 备:HoroscopeJS(JS,本命盘) | Python备:kerykeion(Python) | 星盘SVG→render_astrology_svg(name,y,m,d,h,min,lat,lng,city,chart_type,theme)")
                appendLine("【印度吠陀】南印/北印盘→JS首选Caelus(nakshatras/dasha/vargas/yogas,Lahiri) | Python备:jhora(备:stellium.visualization.vedic)")
                appendLine("【塔罗】arcanite(Python,78张+36雷诺曼+牌阵,洗牌抽牌解读) | 深度:抽牌后查777表→Kaabalah.SPHERES[name]/FOUR_WORLDS[name]/HEBREW_LETTERS_DATA[letter]取对应(完整映射表见executor.py塔罗卡巴拉段) | 塔罗无需出生")
                ...
                appendLine("【农历天文】黄历/择日/建除/太岁→cnlunar(备:lunar_python,Lunar(JS)) | 公历农历转换/八字→lunar_python(备:cnlunar,Lunar(JS)) | 二十八宿/宿曜→Lunar(JS).getTwentyEightMans()(备:pyswisseph,cnlunar) | 吉神凶神/彭祖百忌→cnlunar | 生肖/干支/纳音/闰候→bazi_china(bazi,shengxiao,ganzhi,yue)(备:lunar_python) | 节气/天文→lunar_python(备:cnlunar,pyswisseph)")
                appendLine("输入要求：八字/紫微/占星/吠陀需生日时辰+性别+经纬度 | 合盘需双人生日 | 黄历/择日/太岁/节气仅需日期 | 六爻/梅花/太玄/荆诀/塔罗无需出生")
                appendLine("⚠️ JS引擎通过 eval_javascript 调用。首次需 load: action='load', library='<库名>'（库名见 executor.py 路由表）。10库: qimen-engine | ziwei-nihai | iching-shifa-engine | taixuan-engine | lunar-engine | astronomy-engine | horoscope-engine | kaabalah-engine | caelus-engine(西洋+吠陀占星) | iztro-engine(紫微⭐3841)")

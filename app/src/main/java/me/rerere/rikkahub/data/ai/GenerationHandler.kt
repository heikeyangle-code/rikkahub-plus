    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("")
                appendLine("  各系统 params 格式:")
                appendLine("    塔罗: {spread, seed, question_type?, kaabalah?}")
                appendLine("    雷诺曼: {spread, seed}")
                appendLine("    八字: {year, month, day, hour, gender}")
                appendLine("    紫微: {year, month, day, hour, gender, engine?}")
                appendLine("    现代西洋占星: {year, month, day, hour, tz, lat, lon, depth?}")
                appendLine("       (心理/成长取向:十大行星+相位+格局+合盘)")
                appendLine("    传统西洋占星: {year, month, day, hour, tz_offset, lat, lon}")
                appendLine("       (事件判断取向:本质尊贵+主限向运+阿拉伯点+互容接纳)")
                appendLine("    吠陀: {year, month, day, hour, tz, lat?, lon?, depth}")
                appendLine("       depth='standard'=PyJHora+NodeJhora, depth='deep'=全引擎+分盘")
                appendLine("       吠陀engine: PyJHora(主力,54Dasha/Panchanga/Yoga774) + NodeJhora(DE440/Jaimini/Shadbala)")
                appendLine("    人类图: {year, month, day, hour, tz, gene_keys?}")
                appendLine("    灵数卡巴拉: {year, month, day, word?, feature}")
                appendLine("    奇门三式: {year, month, day, hour?, feature}")
                appendLine("    六爻梅花: {method, seed?, year?, month?, day?}")
                appendLine("")
                appendLine("第2步 — 读解读模板:")
                appendLine("  首次遇到某系统时调 mingli_guide(system='系统名') → 获取解读框架/话术/规范")
                appendLine("  之后可缓存,无需重复读取")
                appendLine("")
                appendLine("第3步 — 组织回复:")
                appendLine("  先用 mingli 数据(确定性), 再按 mingli_guide 模板组织语言")

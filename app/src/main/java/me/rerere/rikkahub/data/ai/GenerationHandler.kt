    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【塔罗】arcanite(Python) — 统一规则:先结论后解释|故事优先不罗列数据|牌必须串联|数据只增强语气不罗列。输出:【问题】【牌阵】【一句话答案】【主题】【整体故事】【逐牌(位置+状态+心理解释+前后牌关系+符号点缀,每张3~5句)】【牌阵结构(元素倾向+大牌比例+重复主题+关系网络)】【结论】【建议≤3】【反思问题】【箴言】。数据:必须(core_meanings/position_interpretations/question_contexts/card_relationships/journaling_prompts/affirmations/meditation_focus)润色(symbols/element/astrology)结构(element_balance/major_arcana_ratio/court_card_ratio/repeated_numbers/repeated_suits)隐藏(777/hebrew_letters/tree_of_life/four_worlds/sephiroth)。Pro→+结构分析|Master→+秘传分析(777/生命之树)。深度→查777表→Kaabalah。无需出生")
                appendLine("【雷诺曼】LenormandDeck.load()→draw_with_data(N)返回LenormandDrawnCard对象,一步拿牌+数据(item.card_id/item.get_core()/item.get_combination_with()...)。语义getter:get_core/get_timing/get_as_person/get_modifier_behavior/get_playing_card/get_topic_contexts/get_line_reading/get_combination_grammar/get_combinations/get_grand_tableau(禁止_data裸访问)。组合:item.get_combination_with(other.card_id,position=left/right)→自动方向+语法回退。统计:deck.analyze_draw(items)→正逆位+全正/全逆检测。输出:现实事件模拟器:【问题】【一句话答案】【牌组】【事件故事(箭头流程)】【组合链(A+B→意义/B+C→推进/C+D→结果)】【结论】【建议≤3】。数据:必须(core/keywords/combination_rules/modifier_behavior/line_reading)润色(timing)隐藏(playing_cards→Master附录)as_person(抽到人物卡激活)。Pro→+话题分析/方向/速度|Master→+Grand Tableau(指示牌/近远法/镜像/对角/宫位/扑克牌)。牌阵:list_spreads(system=\\\"lenormand\\\")→line-3/line-5/line-7(7张)/line-9(9张)/grand-tableau(36张)/box-3x3(9张)/cross(5张)/astrological-houses(12张)/relationship(5张) | load_spread(id,system=\\\"lenormand\\\")。模式:默认|Pro(深入)|Master(大师/777)。引擎:FE.parse_karmic_mirrors(line7/9/cross/rel必开)+FE.parse_portrait_3x3_cage(box3x3必开)+FE.parse_grand_tableau_master_mode(GT Master必开)+FE.calculate_house_chaining(追问原因)/FE.calculate_counting_pulse(年运大趋势)按需取。引擎输出=硬骨架,LLM不篡改。无需出生")

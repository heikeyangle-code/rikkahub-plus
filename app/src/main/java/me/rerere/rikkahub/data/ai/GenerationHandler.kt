    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("⚠️ 命理任务，先 import executor，然后用 doc.index() 定位系统标记，只读对应段：")
                appendLine("  八字/四柱/大运 → doc[doc.index('【双库并联】'):doc.index('【奇门三式】')]    (排盘: lunar_python Solar→八字排盘+大运流年+节气 | 解盘: bazi_china神煞/纳音/藏干/十神/古诀/调候/命宫/日主/禄/流月/干支关系 · 黄历/择日/luohou九宫飞星 · 二十八宿/建除/太岁/彭祖 · 生肖合婚)")
                appendLine("  紫微斗数 → doc[doc.index('紫微斗数'):doc.index('【奇门三式】')]    (Iztro JS⭐3841原版 · ziwei_paipan Python · ZiweiNihai倪海夏, 三引擎确定性算法对照)")
                appendLine("  塔罗 → doc[doc.index('╔════ 塔罗'):doc.index('╚════ 塔罗')+1]    (arcanite Python 78张+11牌阵+9层数据+EE元素尊贵 · TarotKit JS中英双语5字段 · Waite原版画面desc+占卜意义)")
                appendLine("  雷诺曼 → doc[doc.index('╔════ 雷诺曼'):doc.index('╚════ 雷诺曼')+1]    (arcanite 36张 · Greer两步法+语法体系 · 扑克插片字典 · FE引擎6方法:parse_karmic_mirrors/GT_portrait/骑士步/镜像/反射/mod/intersection)")
                appendLine("  西洋占星 → doc[doc.index('【西洋占星】'):doc.index('【印度/吠陀】')]    (NatalEngine:本命+合盘+ACG+时区+文本 · Caelus 231函数:本命18/推运7/合盘3/行运12/恒星2/天文5/宫位12制式 · Astronomy VSOP87仲裁)")
                appendLine("  印度占星普通 → doc[doc.index('【印度/吠陀】'):doc.index('【印度占星深度版')]    (NatalEngine吠陀 Rasi/27宿/Dasha · Caelus 3大运/4Yoga/7分盘/27宿/岁差/尊贵)")
                appendLine("  深度版NodeJhora → doc[doc.index('【印度占星深度版'):doc.index('【人类图')]    (DE440 · Shadbala · Ashtakavarga · Jaimini · KP · Transit · Yogini+Narayana大运 · 8Lagna · Upagraha · Panchanga)")
                appendLine("  人类图/基因钥匙 → doc[doc.index('【人类图/Human Design】'):doc.index('【塔罗/雷诺曼')]    (NatalEngine:类型/权威/能量中心/通道/闸门/轮回交叉/circuitAnalysis · 基因钥匙activationSequence)")
                appendLine("  灵数/卡巴拉/Gematria/Ifá → doc[doc.index('【灵数学/卡巴拉/数秘】'):doc.index('【农历/干支/天文】')]    (Kaabalah:LifePath/个人年/挑战/Fibonacci/Gematria/Ifá/生命之树SPHERES+FOUR_WORLDS+HEBREW_LETTERS+777)")
                appendLine("  奇门/大六壬/小六壬 → doc[doc.index('【奇门三式】'):doc.index('【象数易】')]    (QimenEngine 7局法4流派 · LiuRen JS+原子函数 · kinliuren · 小六壬掌诀)")
                appendLine("  六爻/梅花/太玄/荆诀 → doc[doc.index('【象数易】'):doc.index('【西洋占星】')]    (ichingshifa 6种起卦 · IchingShifa双引擎 · meihua_yi Compute+体用 · taixuanshifa+TaixuanLib · jingjue)")
                appendLine("  农历/天文/luohou → doc[doc.index('【农历/干支/天文】'):doc.index('【查询路由】')]")
                appendLine("输入要求：八字/紫微需生日时辰+性别 | 西洋占星/吠陀需生日+经纬度 | 人类图需生日(无需经纬度) | 黄历/择日/太岁/节气仅需日期 | 六爻/梅花/太玄/荆诀/塔罗无需出生")
                appendLine("⚠️ JS引擎通过 eval_javascript 调用。首次需 load: action='load', library='<库名>'。15库: qimen-engine | ziwei-nihai | iching-shifa-engine | taixuan-engine | lunar-engine | astronomy-engine | horoscope-engine | kaabalah-engine | caelus-engine(西洋+吠陀占星) | caelus-birth(时区→UT转换) | iztro-engine(紫微⭐3841) | natalengine-engine(西洋+吠陀+人类图) | node-jhora-engine(印度占星深度版) | tarotkit-engine(塔罗,中英双语) | liuren-engine(大六壬)")

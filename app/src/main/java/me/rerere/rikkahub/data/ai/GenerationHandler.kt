    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【塔罗】from tarot_elemental_engine import ElementalDignityEngine as EE; from arcanite.core.spread import list_spreads, load_spread | from arcanite.core import TarotDeck; d=TarotDeck.load(system=\\\"tarot\\\"); cardsrds=d.draw(N) | 代理:cards[i].get_core_meaning()/get_affirmations()/get_journaling_prompts()/get_symbols() | 牌阵:list_spreads()→single-focus/past-present-future/mind-body-spirit/situation-action-outcome/five-card-cross/four-card-decision/relationship-spread/horseshoe-traditional/horseshoe-apex/celtic-cross/year-ahead | EE全字段+秘传附录按需取 | 统一规则:先结论后解释|故事优先不罗列数据|牌必须串联|数据只增强语气不罗列。输出:【问题】【牌阵】【一句话答案】【主题】【整体故事】【逐牌(位置+状态+心理解释+前后牌关系+符号点缀,每张3~5句)】【牌阵结构(元素倾向+大牌比例+重复主题+关系网络+数字学母题+正逆位信号(仅高比例逆位时提及)])【结论】【建议≤3】【反思问题】【箴言】。数据:必须(core_meanings/position_interpretations/question_contexts/card_relationships/journaling_prompts/affirmations/meditation_focus)润色(symbols/elemental_correspondences共10项)结构(EE.full_analysis全字段:statistics(counts/dominant/deficient/各元素占比) / composition.major_arcana_ratio / composition.court_card_ratio / composition.repeated_numbers / composition.repeated_suits / numerology / absence / doubling)秘传附录(Kaabalah按需取,不在正文展开)。无需出生\")")
                appendLine("【雷诺曼】from lenormand_engine import LenormandFateEngine as FE; from arcanite.core.spread import list_spreads, load_spread | from arcanite.core import LenormandDeck; d=LenormandDeck.load()→draw_with_data(N)返回LenormandDrawnCard对象,一步拿牌+数据(item.card_id/item.get_core()/item.get_combination_with()...)。语义getter:get_core/get_timing/get_as_person/get_modifier_behavior/get_playing_card/get_topic_contexts/get_line_reading/get_combination_grammar/get_combinations/get_grand_tableau(禁止_data裸访问)。组合:item.get_combination_with(other.card_id,position=left/right)→自动方向+语法回退。统计:电荷属性分布(positive/neutral/negative占比)+速度牌占比(fast/medium/slow)+人物卡激活检测。输出:现实事件模拟器:【问题】【一句话答案】【牌组】【事件故事(箭头流程)】【组合链(A+B→意义/B+C→推进/C+D→结果)】【结论】【建议≤3】。数据:必须(core/keywords/combination_rules/modifier_behavior/line_reading)润色(timing)隐藏(playing_cards→GT扑克牌附录)as_person(抽到人物卡→展开角色描写)。Pro→+话题分析/方向/速度|GT专属+Step1内九宫格定调→Step2 MOD近远法[含speed权重+方向]→Step3骑士步/镜像/反射[仅指示牌深挖]→Step4宫位背景[落宫+级联链]+交叉法+扑克牌附录。牌阵:list_spreads(system=\\\"lenormand\\\")→line-3/line-5/line-7(7张)/line-9(9张)/grand-tableau(36张)/box-3x3(9张)/cross(5张)/astrological-houses(12张)/relationship(5张) | load_spread(id,system=\\\"lenormand\\\")。模式已统一不分层。引擎:FE.parse_karmic_mirrors(所有有mirror_target的牌阵必开)+FE.parse_portrait_3x3_cage(box3x3/GT必开)+FE.parse_grand_tableau_master_mode(GT专属,内含Step1-4全套SOP)+FE.calculate_knights_move(任意牌暗线)+FE.get_gt_mirrors(GT镜像)+FE.get_reflection(GT反射,独立调用)+FE.get_inner_9_ring(局部九宫格深挖)+FE.get_intersection(行列交叉)+FE.calculate_mod(主题牌权重排序)+FE.calculate_house_chaining(追问原因)/FE.calculate_counting_pulse(年运大趋势)按需取。引擎输出=硬骨架,LLM不篡改。无需出生\")")

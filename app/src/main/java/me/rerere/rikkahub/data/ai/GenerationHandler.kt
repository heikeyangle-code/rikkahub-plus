    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("    紫微(紫微斗数/紫薇/ziwei): {year, month, day, hour, gender, engine?}")
                appendLine("    现代西洋占星(现代占星/星座/western_astro): {year, month, day, hour, tz, lat, lon}")
                appendLine("    传统西洋占星(古典占星/卜卦/horary/traditional_astro): {year, month, day, hour, tz_offset, lat, lon}")
                appendLine("    吠陀(印度占星/jyotish/vedic): {year, month, day, hour, tz, lat, lon}")
                appendLine("    人类图(human_design/humandesign): {year, month, day, hour, tz, gene_keys?, transits?}")
                appendLine("    灵数卡巴拉(生命灵数/卡巴拉/kabbalah): {year, month, day, word?, feature}")
                appendLine("    奇门(奇门遁甲/奇门三式/qimen): {year, month, day, hour?, feature}")

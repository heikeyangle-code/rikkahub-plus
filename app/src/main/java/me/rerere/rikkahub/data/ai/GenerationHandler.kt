    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【西洋占星】NatalEngine(JS,本命盘+合盘,已解读输出bigThree/行星/宫位/相位/元素) — 主力解读 | 互补:HoroscopeJS(JS,图表/宫位制) Caelus(JS,原始行星位置) Astronomy(JS,日食月食/行星升降) | Python C扩展全删")
                appendLine("【印度吠陀】NatalEngine(JS,Lahiri岁差+27星宿+pada+Rashi+大运) — 主力解读 | 互补:Caelus(JS,原始行星位置) Astronomy(JS,行星数据验证)")

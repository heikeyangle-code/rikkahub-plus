    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【奇门三式】奇门遁甲→问用户选kinqimen(2局法)或QimenEngine(7局法+断语)或都跑对照 | 大六壬→kinliuren | 太乙神数→kintaiyi | 皇极经世→kinwangji | 小六壬→lunar_python取月日时→掌诀推算(大安留连速喜赤口小吉空亡)")
                ...
                appendLine("【西洋占星】本命盘→问用户选kerykeion(Python,SwissEphemeris毫角秒)或HoroscopeJS(JS,Kepler+7宫位制)或都跑 | 行星/日食→pyswisseph(备:Astronomy(JS),VSOP87角秒) | 深析/推运/合盘→仅Python(stellium/flatlib/immanuel) | 注:HoroscopeJS不带日食,Astronomy不带排盘 | 星盘SVG→render_astrology_svg(name,y,m,d,h,min,lat,lng,city,chart_type,theme)")
                ...
                appendLine("⚠️ JS引擎通过 eval_javascript 调用。首次需 load: action='load', library='<库名>'（库名见 executor.py 路由表）。8库: qimen-engine | ziwei-nihai | iching-shifa-engine | taixuan-engine | lunar-engine | astronomy-engine | horoscope-engine | kaabalah-engine")

    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【雷诺曼】arcanite(system=lenormand) — 现实事件模拟器。输出:【问题】【一句话答案】【牌组】【事件故事(箭头流程)】【组合链(A+B→意义/B+C→推进/C+D→结果)】【结论】【建议≤3】。数据:必须(core/keywords/combination_rules/modifier_behavior/line_reading)润色(timing)隐藏(playing_cards→Master附录)as_person(抽到人物卡激活)。Pro→+话题分析/方向/速度|Master→+Grand Tableau(指示牌/近远法/镜像/对角/宫位/扑克牌)。牌阵:from arcanite.core.spread import list_spreads,load_spread。塔罗11阵(single-focus/celtic-cross等)|雷诺曼2阵(line-3/line-5)。模式:默认|Pro(深入)|Master(大师/777)。无需出生")

...
class GenerationHandler(
    ...
    fun describeTool(name: String): String = when {
        name.startsWith("github_") -> "正在操作 GitHub..."
        name.startsWith("execute_python") -> "正在执行 Python 代码..."
        name.startsWith("execute_command") -> "正在执行 Shell 命令..."
        name.startsWith("file_") -> "正在操作文件..."
        name.startsWith("data_process") -> "正在处理数据..."
        name.startsWith("database_") -> "正在查询数据库..."
        name.startsWith("search_web") || name.startsWith("scrape_") -> "正在搜索..."
        name.startsWith("convert_file") -> "正在转换格式..."
        name.startsWith("create_asset") -> "正在生成素材..."
        name.startsWith("use_skill") -> "正在读取知识..."
        name.startsWith("clipboard") -> "正在操作剪贴板..."
        name.startsWith("get_time") -> "正在获取时间..."
        name.startsWith("text_to_speech") -> "正在朗读..."
        name.startsWith("present_file") -> "正在分享文件..."
        name.startsWith("eval_javascript") -> "正在执行 JS..."
        name.startsWith("memory_") -> "正在处理记忆..."
        else -> "正在处理 $name..."
    }

    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("    八字: {year, month, day, hour, gender, feature?}")
                appendLine("       feature可选: shengxiao(生肖配对,跳过排盘) | luohou(择日九宫飞星,跳过排盘) | all(全量)")
                ...
                appendLine("    吠陀: {year, month, day, hour, tz, lat?, lon?}")
                appendLine("       (PyJHora+NodeJhora+NatalEngine+Caelus 四引擎全开)")
                appendLine("       (含行运预测varsha_pravesh+8瑜伽+19saham+日月食+寿命)")
                appendLine("    人类图: {year, month, day, hour, tz, gene_keys?, transits?}")
                appendLine("    灵数卡巴拉: {year, month, day, word?, feature}")
                appendLine("    奇门三式: {year, month, day, hour?, feature}")
                appendLine("    六爻梅花: {method, seed?, year?, month?, day?, feature?}")
                appendLine("")

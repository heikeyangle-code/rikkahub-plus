    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("prettytable 已安装，用 execute_python 调用来格式化表格数据")
                appendLine("from prettytable import PrettyTable")
                appendLine("适用场景：仅在纯文本/终端环境需要清晰表格边框时使用")
                appendLine("注意：聊天界面支持 Markdown 表格渲染，优先用 Markdown 表格（更美观）")
                appendLine("prettytable 仅用于 execute_python 输出大量结构化数据且纯文本可读性差的情况")
                appendLine("</formatting>")
                appendLine()

    ...
    suspend fun buildCachedSystemPrompt(
        ...
        val assemblerContext = me.rerere.rikkahub.data.ai.prompts.PromptContext(
            ...
            leadInInstructions = buildString {
                ...
                appendLine("【西洋占星】NatalEngine.calculateAstrology(日月升+文本+元素平衡,字段全) + Caelus(本命14个函数/7种推运/12宫位制/3种合盘/行运/恒星/天文事件) + Astronomy仅做高精度日月食备选。Caelus: Engine→chartAt→interpretationContext。AI必须读executor路由按步骤调。| HoroscopeJS已被Caelus覆盖")

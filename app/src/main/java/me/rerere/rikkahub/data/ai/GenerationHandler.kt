...
class GenerationHandler(
    ...
    fun describeTool(name: String): String = when {
        ...
        name.startsWith("execute_python") -> "执行 Python 代码..."
        name.startsWith("execute_command") -> "执行 Shell 命令..."
        name.startsWith("file_") -> "操作文件..."
        name.startsWith("data_process") -> "处理数据..."
        name.startsWith("database_") -> "查询数据库..."
        name.startsWith("search_web") || name.startsWith("scrape_") -> "搜索..."
        name.startsWith("convert_file") -> "转换格式..."
        name.startsWith("create_asset") -> "生成素材..."
        name.startsWith("use_skill") -> "读取知识..."
        name.startsWith("clipboard") -> "操作剪贴板..."
        name.startsWith("get_time") -> "获取时间..."
        name.startsWith("text_to_speech") -> "朗读..."
        name.startsWith("present_file") -> "分享文件..."
        name.startsWith("eval_javascript") -> "执行 JS..."
        name.startsWith("memory_") -> "处理记忆..."
        else -> "处理 $name..."
    }

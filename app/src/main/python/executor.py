...
【印度占星深度版 · NodeJhora — JPL DE440 星历, 纯 JS】
  ...
  ⚠️ 加载约需 10-30 秒 (42MB 引擎), timeout 设 60s。
  ...
  ⚠️ YogaEngine.findYogas 首次调用较慢 (YOGA_LIBRARY 规则量大)。
  ⚠️ generateVimshottari depth=3 递归量大, 建议 depth=2 按需展开。

  ╔═══════════════════ 调用骨架 ════════════════════╗
  ║ dt=NodeJhora.DateTime.fromISO("1990-06-15T12:00:00+08:00") ║
  ║ e=NodeJhora.EphemerisEngine.getInstance()         ║
  ║ p=e.getPlanets(dt,{lat,lon},{ayanamsaOrder:1})  ║
  ║ jd=e.julday(dt)                                  ║
  ║ h=e.getHouses(jd,lat,lon,"W",true)               ║
  ...
  NodeJhora.NodeJHora.calculate(new Date("1990-06-15T12:00:00+08:00"),
    ...
    → {planets, houses, panchanga, ascendant, ayanamsa}
    ⚠️ 返回 Promise, 用 .then(r=>...)
  ...
  NodeJhora.calculatePanchanga(sunLon, moonLon, dt, sunriseHour=6.0)
    → {tithi:{name,index,paksha,remaining_degrees},
       nakshatra:{name,lord,deity,index},
       yoga:{name,index}, karana:{name,index}, vaara:{name,index}}

  ━━━ 三、分盘 / Vargas (D1–D60) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  NodeJhora.calculateVarga(lon, division)  // division 为数字: 1=D1(Rasi), 2=Hora,
    → {sign, signIndex, division}          //   3=Drekkana, 9=Navamsa, 10=Dasamsa,
  便捷别名:                                  //   12=Dvadasamsa, 30=Trimsamsa, 60=Shashtyamsa
  NodeJhora.calculateD9(lon) / NodeJhora.calculateD10(lon) / NodeJhora.calculateD60(lon)
  ...
  NodeJhora.generateVimshottari(dt, moonLon, depth=2)
    → [{lord, start, end, years, subPeriods:[{lord,...}]}]
    depth: 1=仅 Maha, 2=Maha+Antar, 3=Maha+Antar+Pratyantar
  NodeJhora.calculateDashaBalance(moonLon)
    → {elapsedYears, remainingYears, proportionElapsed}

  NodeJhora.YoginiDasha.calculate(moonLon, dt, 50)   // 36年周期, 8 Yoginis
    → [{planet, start, end, durationYears, level, subPeriods}]

  NodeJhora.NarayanaDasha.calculate(chart, dt, 80)   // Rasi 序列大运
    → [{signIndex, start, end, durationYears, isForward}]
    chart 需含 {ascendant, planets:[{id,longitude}]}

  ╔══════════════════ Dasha 对比 ═══════════════════╗
  ║ Vimshottari: 120年, 9段, 最常用                  ║
  ║ Yogini:      36年,  8段, 快速审视                ║
  ...
  NodeJhora.calculateShadbala({
    ...
    vargaPositions: [{vargaName:"D9", sign}]  // Navamsa 位置
  })
  → {totalVirupa,        // 总分 (越大越强)
     sthana:  {uchcha, saptavargaja, kendra, ojayugma},
     dig:     digBala,   // 方向力量
     kaala:   {natonata, paksha, tribhaga, ayanabala},
     chesta:  chestaBala, // 视运动力量
     naisargika, drigBala}
  各行星比 totalVirupa → 力量排行
  ...
  NodeJhora.calculateSaptavargajaBala(planetId, rashiSign, vargaPositions)
  NodeJhora.calculateDrigBala(targetPlanet, allPlanets) → 相位力量
  ...
  NodeJhora.Ashtakavarga.calculateSAV(planets)
    → [12] 七曜综合 Sarvashtakavarga, 每宫总分
  SAV[ind] 越高 → 该宫越有力; BAV 看单星在12宫的分布

  ╔══════════════ 力量体系对比 ══════════════════╗
  ║ Shadbala:    行星本身有多强 (6维度)           ║
  ...
  NodeJhora.JaiminiDashas.calculateCharaDasha(ascSignIndex, planets)
    → [{signIndex, start, end, durationYears}]
  ...
  t.findExactAspect(p1Id, p2Id, angle, startDt, endDt, 0.01)
    → [{exactDate, angle}]  精确入相位时刻
    angle: 0/60/90/120/180
  ⚠️ 这两个是 async — 用 .then(r=>{...})

  ━━━ 十一、特殊 Lagna (8种) ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  NodeJhora.calculatePranapada(dt, sunriseDt, sunLon)
    → {longitude, sign, signIndex}
    气息上升点, 用于健康/活力判断

  NodeJhora.calculateInduLagna(ascSign, moonLon)     // ascSign: 1-12
  NodeJhora.calculateShreeLagna(dt, sunriseDt, moonLon)
  NodeJhora.calculateHoraLagna(dt, sunriseDt, ascLon)
  NodeJhora.calculateGhatiLagna(dt, sunriseDt, sunLon)
  NodeJhora.calculateBhavaLagna(dt, sunriseDt, sunLon)
  NodeJhora.calculateVarnadaLagna(ascLon, horaLagnaLon, ascSign)
  均需 sunriseDt: 出生当天日出时刻的 Luxon DateTime
  均返回 {longitude, sign:"Aries", signIndex:0}

  ━━━ 十二、Upagraha (虚星/影星, 5个) ━━━━━━━━━━━━━━━━━━━━━━━

  NodeJhora.calculateTimeUpagrahas(dt, sunriseDt, sunsetDt, sunLon, moonLon)
    → [{name, longitude}×5]
    Dhooma→Vyatipata→Parivesha→Indrachapa→Upaketu (链式推导)
  NodeJhora.calculateDhumadiUpagrahas(sunLon)
    → [{name, longitude}×5]  同上但只依赖日度
  ...
  NodeJhora.D, NodeJhora.NAKSHATRA_SPAN_D: 13.3333...° (D对象)
  NodeJhora.DASHA_YEAR_DAYS: 365.25
  ...
  ⚠️ 自探索: load 后用 Object.keys(NodeJhora) 看全局导出,
  ...
  它能做什么:
    JPL DE440 星历 (NASA 公共领域, 1849–2150)。精度同级 Swiss Ephemeris。
    Shadbala(六力) + Ashtakavarga(八分力) 完整行星力量量化。
    Jaimini(CharaKaraka + CharaDasha + Arudha + RashiDrishti)。
    KP(SubLord 亚主星 + 全盘主星分析 + Ruling Planets)。
    3种大运 + Yoga格局检测 + 行运 + 8种特殊Lagna + 5虚星。
    Caelus/NatalEngine 未覆盖的深度吠陀分析全在这。

  什么时候调:
    任何深度吠陀分析 — Shadbala力量排行 / Ashtakavarga宫位力量
    "哪个星最强/最弱/力量排行" → NodeJhora.calculateShadbala
    "八分力/Ashtakavarga/Sarvastakavarga" → NodeJhora.Ashtakavarga.calculateSAV
    "Atmakaraka/CharaKaraka/Jaimini" → NodeJhora.JaiminiCore.calculateCharaKarakas
    "CharaDasha/Jaimini大运" → NodeJhora.JaiminiDashas.calculateCharaDasha
    "Arudha/ArudhaLagna/投射盘" → NodeJhora.JaiminiCore.calculateArudha
    "KP/SubLord/亚主星/星宿主星" → NodeJhora.KPSubLord.calculateKPSignificators
    "KP主宰星/择时" → NodeJhora.KPRuling.calculateRulingPlanets
    "Yogini大运/36年周期" → NodeJhora.YoginiDasha.calculate
    "Narayana大运/Rasi大运" → NodeJhora.NarayanaDasha.calculate
    "Yoga/格局/富贵贫贱" → NodeJhora.YogaEngine.findYogas
    "行运/某星何时换座/入相位" → NodeJhora.TransitEngine
    "Pranapada/InduLagna/特殊上升" → NodeJhora.calculatePranapada 等
    "虚星/Dhooma/Vyatipata" → NodeJhora.calculateTimeUpagrahas
    "行星关系/敌友/临时关系" → NodeJhora.getRelationship

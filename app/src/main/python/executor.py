...
(当前时刻的行运闸门)
 ...
 arcanite            →  塔罗: from arcanite.core import TarotDeck; d=TarotDeck.load(system="tarot"); cards=d.draw(N); [print(c.card_id,c.card_name,c.orientation.value) for c in cards]
                       ...
                       【塔罗输出】塔罗·生命故事
                         来源:
                           Eden Gray → Fool's Journey
                           Joan Bunning → 3步解读法(直觉→含义→位置); 能量水位看逆位
                           Mary K. Greer → 花色叙事流; 宫廷牌=人物/态度
                           Benebell Wen → 概览→分析→综合

                         核心流程（Joan Bunning Lesson 11: 3 层交织）:
                           ① 你的直觉反应——看到牌的第一感受
                           ② 牌的传統含义——前人积累的知识
                           ③ 位置的含义——这张牌在牌阵中的角色
                           三者交织，缺一不可

                         ── 开读（看阅读类型和牌数，灵活选择下面格式）──
                           阅读类型（Joan Bunning分类）:
                             Daily Reading(1-3张)→ 快速点睛，只出印象+核心牌+建议
                             Question Reading(具体问题)→ 按位置逐张解，以答案为终点
                             Other Reading(为他人看)→ 用「对方」口吻，注意边界
                             Open Reading(开放式)→ 找出最扎眼的一张牌作为起点
                           牌阵结构（Joan Bunning: 先找最吸引你注意力的牌）:
                             「当这些牌放下时，我的目光立刻被【XX】吸引——它是整局的心脏」
                             从这张牌开始，让故事像水一样流出
                             不是每局都走完所有步骤——有的阅读一张牌就说明了一切

                         ── A. 总体印象（Benebell Wen 概览）──
                           【问题】用户原问
                           【牌阵】名称 + 位置含义
                           【一眼直觉】第一眼看到什么？
                             大牌出现→ 这段能量是深层/决定性的（Joan Bunning: 大牌=深层长期，小牌=日常周期）
                             大牌→ 标注Fool's Journey阶段（Eden Gray创始）
                             元素谁主宰→ 火多就行动期，水多就情绪期
                             正逆位比例→ 流畅还是有阻塞
                             重复数字/花色→ 什么主题被强调
                           话术:
                             "大牌占了一半——这不仅仅是一周的事，这是一段旅程"
                             "权杖三连出现——你现在处在『行动加速』的能量里"

                         ── B. 逐牌解读（Joan Bunning: 3层交织法）──
                           每张牌:
                             位置名｜牌名
                             第一步·画面直觉: TarotKit description 描述画面 + 你的第一感觉
                             第二步·传统含义: coreMeaning(coreKeyword + archetype)
                               大牌→加上Fool's Journey阶段
                               宫廷牌→ Mary K Greer: 代表人物特征/态度/成熟度，也可能是行动
                             第三步·位置含义: position_interpretations(按rag_mapping取)
                             融合: 把三步拧成 2-4 句自然的话，不说术语
                             逆位→ 用能量水位看(Joan Bunning: 正位充分展现，逆位在较低水平)
                           话术:
                             "抽到这张【皇后】的时候，'丰饶'这个词跳进我脑子里。她在画中坐在丰饶的大地上——你此刻也处在这样一个『扎根生长』的阶段"
                             "这是一张宫廷牌——它可能代表你身边的某个人，或者你自己正在扮演的『母亲/照顾者』角色"
                             "逆位的【太阳】不是开心不见了，是光芒被遮了一层——开心还在，但没之前那么亮"

                         ── C. 花色叙事流（Mary K. Greer: Tarot Flow——花色逐卡讲故事）──
                           当同一花色出现多张时，按数字顺序读它们在讲什么故事（Greer原话: "花色本身随着卡牌推进讲述故事"）
                           话术:
                             "你的牌阵里出现了钱币2→钱币5→钱币9——这是一条完整的故事线"

                         ── D. 牌阵互动（Joan Bunning 成对阅读 + 元素对话）──
                           成对阅读:
                             找最显眼的一对，检查是否系统预定义的对立或强化关系
                             对立对→ 揭示冲突/选择：愚者的对立牌=教皇/死神/恶魔/宝剑二/钱币四
                             强化对→ 确认主题：愚者的强化牌=倒吊人/星星/审判/权杖三
                             宫廷对→ 人物关系动态
                           Card Sandwich(三牌连读辅助):
                             相邻三张: 左=背景, 中=当前状态, 右=方向
                           EE元素尊贵 + Mary K Greer反应规则:
                             Fire+Fire=加速, Fire+Air=燎原, Fire+Water=蒸汽(张力)
                             Water+Earth=孕育, Air+Earth=干涸, 等
                             架桥=某元素连续3+张, 孤岛=元素独一张
                           话术:
                             "你的牌阵中出现了愚者+他的对立牌死神——这告诉我你正在『开始与结束』之间拉扯"
                             "元素方面，火+火的组合贯穿全局——行动力很强但容易过热，需要水元素来降温"
                         ...
                         ── F. 落幕与回响 ──
                           【结论】一句话核心洞见
                           【建议】≤3条（readingAspects.advice + affirmations）
                           【反思问题】1条 journaling_prompts，留给用户自己去想
                           【一句话留给你】一句能带走的话
                       ...
                       【核心理念】Joan Bunning: 逆位=能量尚未完全展开，或正在减弱
                       不是"反义"，是"能量水位"的信息:
                         正位 → 能量充分显现，品质/力量活跃可用
                         逆位 → 能量在较低水平：可能是早期阶段、正在减弱、或尚未成熟
                         不一定是"负面"——同一个逆位对不同人有不同意义
                         例: 逆位太阳→活力减少而非消失, 逆位皇后→孕育力量存在但水平较低
                       AI解读参考:
                         逆位+大牌 → 这个重大课题的能量目前受阻/未完全激活
                         逆位+小牌 → 日常层面的这个品质正在减弱或发展初期
                         逆位出现在"过去" → 这段能量已经过去了
                         逆位出现在"未来" → 这个能量会以较弱/不同的形式到来
                         逆位出现在"建议" → 需要放弱这个品质，或换一种方式使用它
                       关键: 逆位是天平，不是告示——它衡量能量的深浅

...
(当前时刻的行运闸门)
 ...
 arcanite            →  塔罗: from arcanite.core import TarotDeck; d=TarotDeck.load(system="tarot"); cards=d.draw(N); [print(c.card_id,c.card_name,c.orientation.value) for c in cards]
                       ...
                       ╔══════════════════ 塔罗核心参考字典 ═══════════════╗
                         ...
                         一、花色人格与领域(Suit Personalities):
                           Wands=火→行动者,事业/激情/创造——权杖多=在行动驱动阶段
                           Cups=水→感受者,情感/关系/直觉——圣杯多=情感主导期
                           Swords=风→思考者,冲突/决策/心智——宝剑多=脑内博弈期
                           Pentacles=土→建造者,财富/现实/健康——钱币多=物质聚焦期
                         二、数字成长链(Ace→10每条花色通用的叙事逻辑):
                           Ace=种子/灵感的火花(起始) | 2=对立/选择/平衡(决定方向)
                           3=创造/扩张/初步成果(落实) | 4=稳定/固化/休息(巩固)
                           5=冲突/变化/摩擦(打破僵局) | 6=和谐/胜利/共享(恢复平衡)
                           7=考验/内省/策略(重新评估) | 8=力量/推进/掌控(加速)
                           9=完成/沉淀/独善(接近终点) | 10=饱和/圆满/过度(一个周期结束)
                           用法:同一花色连续数字=这个故事在推进;重复数字=该主题被强烈强调
                         三、宫廷牌判定(判定顺序,按优先级递减):
                           ①真人→牌代表一个真实存在的人(年龄/性别/性格/社会角色)
                           ②人格面具→代表问卜者自己正在扮演的角色或性格侧面
                           ③行为模式→代表一种做事方式或应对策略
                           ④事件/信号→代表与该人物相关的事件即将发生
                           判定依据:看牌阵位置(外部位置更倾向真人)+周围是否有其他人物牌
                         四、宫廷牌层级(解读层,非卡巴拉层):
                           Page=学习/探索/接收信息(学生/信使/初学者)
                           Knight=行动/追求/战斗(战士/追求者/冒险家)
                           Queen=内化/孕育/滋养(成熟女性/守护者/情感中心)
                           King=掌控/领导/权威(成熟男性/决策者/领域专家)
                         五、逆位体系(Bunning+Greer双轨):
                           Bunning能量水位(已覆盖):逆位=能量尚未完全展开或正在减弱
                           Greer12种逆位(新增):阻塞=能量被卡住/投射=向外归因/内化=转向自我
                            延迟=时间未到/否认=拒绝面对/释放=放下执念/过度=过量使用
                            缺失=缺乏该品质/固着=卡在某个阶段/退化=退回旧模式/爆发=突然释放
                            转化:逆位中的正面——阻塞意味着需要停下来
                           用法:①大牌逆位优先用Greer体系(阻塞/投射/转化) ②小牌逆位优先Bunning(能量水位)
                           ③全盘逆位占比>50%时=整体能量受阻滞,不一定全凶
                         六、主题识别算法(权重评分):
                           大牌×2(深层能量) | 中心/指示位×2 | 重复数字×1.5
                           重复花色×1.2 | 宫廷牌提示人物主题
                           求和后最高分=该牌阵的核心主题牌
                         七、元素过载规则:
                           某元素占比>50%时=该领域过度专注,需要补相反元素
                           Wands过载=冲动/burnout | Cups过载=情绪淹没
                           Swords过载=过度思考/焦虑 | Pentacles过载=物质固化/僵化
                           补法:找全阵中缺失的元素对应的牌作为建议方向
                         八、三牌以上叙事算法:
                           三牌:左=起因→中=发展→右=结果
                           四牌:依次=背景→冲突→转折→落点
                           五牌:依次=设定→张力→高潮→回落→收束
                           大于五张:按位置分组(过去组/现在组/未来组),每组内部按以上规则
                         九、位置互动规则:
                           过去→现在→未来:逻辑桥接=背景→当前状况→发展趋势
                           挑战→建议:必须成对读,挑战定义问题,建议提供解法
                           显意识→潜意识:表面知道的事 vs 内心深处真正在运作的事
                           外部→内部:外部环境 vs 内心状态,矛盾点=问卜者最需要面对的张力
                       ╚════════════════════════════════════════════╝

 ...
 arcanite            →  塔罗: from arcanite.core import TarotDeck; d=TarotDeck.load(system="tarot"); cards=d.draw(N); [print(c.card_id,c.card_name,c.orientation.value) for c in cards]
                       ...
                       【塔罗输出】塔罗·生命故事
                         来源:
                           ...
                           Joan Bunning → 3步解读法(直觉→含义→位置); 能量水位看逆位; 成对阅读
                         ...
                         ── 开读 ──
                           阅读类型（Joan Bunning课程分类）:
                             Daily Reading(1-3张)→ 快速点睛，出印象+核心牌+建议
                             Question Reading(具体问题)→ 逐位置解，以答案为终点
                             Other Reading(为他人看)→ 用「对方」口吻
                             Open Reading(开放式)→ 找最扎眼的一张牌作为起点
                           起手（Joan Bunning样局原话）:
                             「当这些牌放下时，我的目光立刻被【XX】吸引——它是整局的心脏」
                             从这张牌开始，让故事流出
                             不是每局都走完所有步骤

                         ── A. 总体印象（Benebell Wen: 先概览，后细读）──
                           【问题】用户原问
                           【牌阵】名称 + 位置含义
                           【一眼直觉】
                             大牌出现→ 这段能量是深层/决定性的
                               小牌=日常周期，大牌=长期课题（Joan Bunning Lesson 12）
                             大牌→ 标注Fool's Journey阶段（Eden Gray, 1970）
                             元素谁主宰→ 火=行动期，水=情绪期
                             正逆位比例→ 能量流畅 / 有阻塞
                             重复数字/花色→ 什么主题被强调
                           话术:
                             "大牌占了一半——这不仅仅是眼前的事，这是一段旅程"
                             "权杖出现了三张——你现在处在一个『行动』的能量里"

                         ── B. 逐牌解读（Joan Bunning: 3层拧成一句话）──
                           每张牌:
                             位置名｜牌名
                             (1) 画面直觉: TarotKit description 描述画面 + 你的第一感受
                             (2) 传统含义: coreMeaning（coreKeyword + archetype）
                                大牌→加Fool's Journey阶段
                                宫廷牌→ Mary K Greer: 代表人物特征/态度/成熟度，也可能是行动
                             (3) 位置含义: position_interpretations(按rag_mapping取)
                            融合: 三步拧成 2-4 句自然的话，不用术语
                            逆位→ Joan Bunning: 正位=充分展现，逆位=在较低水平
                           话术:
                             "抽到【皇后】的时候，'丰饶'这个词跳进我脑子里——你此刻也处在一个『扎根生长』的阶段"
                             "这是张宫廷牌——可能代表你身边的一个人，也可能是你自己正在扮演的『照顾者』角色"
                             "逆位的【太阳】不是开心不见了，是光芒被遮了一层——开心还在，但没那么亮了"

                         ── C. 花色叙事流（Mary K. Greer: "花色本身随卡牌推进讲故事"）──
                           同花色出现多张时，按数字顺序读它们在讲什么故事
                           话术:
                             "钱币2→钱币5→钱币9——同一个领域的三幕剧"

                         ── D. 牌阵互动 ──
                           成对阅读（Joan Bunning: Opposing/Reinforcing Cards）:
                             对立对→ 能量相反，揭示冲突或选择
                               愚者的对立牌: 教皇/死神/恶魔/宝剑二/钱币四
                             强化对→ 能量一致，确认主题
                               愚者的强化牌: 倒吊人/星星/审判/权杖三
                             宫廷对→ 人物关系动态
                             先找最显眼的一对，再构建关系网
                           三牌连读（AI推理手法）:
                             相邻三张: 左=背景, 中=当前, 右=方向
                           EE元素尊贵规则（来自引擎）:
                             Fire+Fire=加速, Fire+Air=燎原, Fire+Water=张力/转化
                             Water+Earth=孕育, Air+Earth=干涸
                             架桥=元素连续3+张, 孤岛=元素独一张
                           话术:
                             "愚者和它的对立牌死神同时出现——你在『开始』和『结束』之间拉扯"
                             "火+火贯穿全局——行动力强但容易过热"

...
输入要求列：生日=公历日期+时辰+性别，日期=只要日期年月日。
  ...
  NatalEngine(主力,唯一带文本解读) → calculateAstrology(本命盘+bigThree+行星+宫位+相位+元素) + compareAstrology(合盘) + calculateVedic(吠陀) + calculateHumanDesign(人类图) + renderChartCardSVG(可视化) + astrocartography(占星地图) + calculateBirthPositions(原始行星位置)

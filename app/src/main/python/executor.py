 ...
 雷诺曼         →  arcanite(system="lenormand") 36张; 数据层:
                       ...
                       ⚠️ print仅取数据。解读正文必须写在回复里，不准在Python里print解读
                        ...
                        ╔══════════════════ 雷诺曼扑克插片参考字典 ═══════════════╗
                          ...
                          数字含义(仅含6-10与A无2345,仅高级阵法中抓同频共振):
                            A=绝对焦点/源头 | 6=宿命感/深根蒂固(如十字架/塔) | 7=琐碎/短时交流(如老鼠/鸟)
                            8=群体/外界互动(如花园/月亮) | 9=极端动静/位移或停滞(如骑士/锚/棺材) | 10=大体量/商业广阔度(非结局,如熊/狗/船)
                          注:28男人(♥A)与29女人(♠A)作为核心指示牌时,A的数字属性豁免不解读
                        ...
                        ╔══════════════════ 雷诺曼输出模板(权威版) ═══════════════╗
                          ...
                          【牌阵结构总结】— 电荷分布(正/中/负张数+占比)+速度牌分布(fast/neutral/slow张数)+人物卡激活清单(牌名+角色)+重复花色/重复数字(若有则标注:同花色=该领域被强调;吉凶大方向:♥大吉/♦偏吉/♠中性/♣大凶,详见【雷诺曼扑克插片参考字典】;完全缺失的花色=被回避/未触及的领域;同数字=该数值主题被强调;点数相加=A+A=2天~K+K=26周)

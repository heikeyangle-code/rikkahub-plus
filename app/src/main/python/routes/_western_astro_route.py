...
def _western_astro(year,month,day,hour,tz,lat,lon,depth="standard"):
    ...
    c=_js("caelus-engine","var e=new Caelus.Engine(Caelus.embeddedData);var jd=Caelus.isoToJd('%s');var chart=e.chartAt(jd,%f,%f,{});var ascIdx=chart.angles.ascendant.sign_index||0;var fortuneLon=0;try{fortuneLon=Caelus.lots(e,jd,%f,%f).fortune.lon;}catch(e){}var bodies_={};if(chart.bodies)chart.bodies.forEach(function(b){bodies_[b.id||b.name]=b});var cusps_=Caelus.housesPlacidus(e,jd,%f,%f);var natal_={bodies:bodies_,cusps:cusps_,zodiac:'tropical'};var transitJd_=jd+365;JSON.stringify({signature:Caelus.chartSignature(chart),patterns:Caelus.detectPatterns(chart),bodies:chart.bodies,cusps:chart.cusps,angles:chart.angles,lots:Caelus.lots(e,jd,%f,%f),isDay:Caelus.isDayChart(e,jd,%f,%f),voidOfCourse:Caelus.voidOfCourse(e,jd)})"%(iso_date,lat,lon,lat,lon,lat,lon))
    result["caelus"]=c

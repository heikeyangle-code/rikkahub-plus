...
def _western_astro(year,month,day,hour,tz,lat,lon,minute=0):
    ...
    try:
        result["synastry"]=_js("natalengine-engine",f"JSON.stringify(NatalEngine.compareAstrology({natal},{natal}))")
    except: pass
    # Caelus: 本命盘(宫位+逆行+尊贵) standard即提供
    _js_load("caelus-engine")
    c=_js("caelus-engine","var e=new Caelus.Engine(Caelus.embeddedData);var jd=%s;var chart=e.chartAt(jd,%f,%f,{});var ctx=Caelus.interpretationContext(chart);JSON.stringify({signature:Caelus.chartSignature(chart),patterns:Caelus.detectPatterns(chart),bodies:chart.bodies,cusps:chart.cusps,angles:chart.angles,lots:Caelus.lots(e,jd,%f,%f),isDay:Caelus.isDayChart(e,jd,%f,%f),voidOfCourse:Caelus.voidOfCourse(e,jd),chartBrief:Caelus.chartBrief(ctx)})"%(jd,lat,lon,lat,lon,lat,lon))
    result["caelus"]=c

...
def _qimen(year,month,day,hour=None,minute=0,feature="all"):
    ...
    if feature in ("qimen","all"):
        ...
        if hour is not None:
            result["qimen_hourly"]=_js("qimen-engine",f"var b=QimenEngine.generate({{type:'rijia',year:{year},month:{month},day:{day}}});JSON.stringify(QimenEngine.generate({{type:'shijia',year:{year},month:{month},day:{day},hour:{hour},minute:{minute},baseChart:b}}))")

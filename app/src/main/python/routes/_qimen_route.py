...
def _qimen(year,month,day,hour=None,minute=0,feature="all"):
    ...
    if feature in ("qimen","all"):
        ...
        result["qimen"]=json.loads(_js("qimen-engine",f"JSON.stringify(QimenEngine.generate({{type:'rijia',year:{year},month:{month},day:{day}}}))"))
        result["engine"]="QimenEngine"
        if hour is not None:
            result["qimen_hourly"]=json.loads(_js("qimen-engine",f"var b=QimenEngine.generate({{type:'rijia',year:{year},month:{month},day:{day}}});JSON.stringify(QimenEngine.generate({{type:'shijia',year:{year},month:{month},day:{day},hour:{hour},minute:{minute},baseChart:b}}))"))
            result["engine"]+="+shijia"
        # 解读层: 格局+星门神详解+十干克应+运筹+长生
        result["qimen_analysis"]=json.loads(_js("qimen-engine",
    ...
    if feature in ("liuren","all"):
        ...
        result["liuren"]=json.loads(_js("liuren-engine",f"JSON.stringify(LiuRen.getLiuRenByDate(new Date({year},{month-1},{day},{hour or 12},{minute})))"))

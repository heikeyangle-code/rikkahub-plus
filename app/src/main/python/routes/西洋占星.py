...
﻿══════════════════════════ 速览 ═══════════════════════════╗ ║ NatalEngine
 → 日月升+文本+元素平衡+合盘评分+ACG+HD行运 ║ ║ Caelus
 → 本命(尊贵/格局/互容/7点)+12宫位+7种推运 ║ ║ Caelus
 → 3种合盘(比较/组合中点/戴维森)+行运+恒星 ║ ║ Caelus
 → 赤纬相位+日食月食+越界+空亡+映点+调和盘 ║ ║ CaelusBirth→ 时区名→UT(备选, 同功能走
NatalEngine.timezone) ║ ║ Astronomy → 星座仲裁(双引擎不一致时)+食相秒级精度 ║ ╚═══════════════════════════════════════════════════════════╝ 
...
NatalEngine.calculateAstrology("1990-06-15", hour, tz, lat, lon)
 ...
 → planets:{mercury,venus,mars,jupiter,saturn,uranus,neptune,pluto} 每行星:{sign:{name,element},degree,longitude}
 ⚠️ 无宫位/逆行/尊贵
 → nodes:{north,south}
 → allAspects:[{a,b,aspect,orb}] 精度: Meeus算法, VSOP87 — Moon误差0.00″
...
── HD 行运 
── NatalEngine.calculateHDTransits(hdResult, "2026-06-22", tz) → {activatedGates,definedCenters,...} NatalEngine.calculateTransitGates("2026-06-22", tz) → {sun:{gate,line}, moon:{...},...} 
── 底层天文 
── NatalEngine.calculateBirthPositions(y, mo, d, h, tz, lat?, lon?) → {sun:{longitude,...},...} NatalEngine.getZodiacSign(longitude) → {name,element,modality,ruler,symbol} 
...
NatalEngine.searchPlaces("Beijing") → [{name,latitude,longitude,timezone,countryCode}] ⚠️ searchPlaces 是 async, 返回 Promise — 用 .then(r=>{...}) 或 await 
── ③ Caelus（深度分析，231+ 函数） 

── 初始化: // 用户报地名→searchPlaces拿时区→resolveUtcOffset拿偏移→拼ISO字符串 var e=new 
Caelus.Engine(
Caelus.embeddedData); var jd=
Caelus.isoToJd("1990-06-15T12:00:00+08:00"); // +08:00是示例, isoToJd内部转UT; 已知UT可直接用 julianDay(y,m,d,h,m,s) var natalJd=jd; var targetJd=
Caelus.julianDay(2026,6,22,12,0,0); var chart=e.chartAt(jd,lat,lon,{}); // 默认Placidus+热带 var ctx=
Caelus.interpretationContext(chart); 
── 本命(18个) 

── chart.bodies.sun
 → {lon,sign,signDeg,house,retrograde,dignities,speed,lat,dist,ra,dec} 可选: sun/moon/mercury/venus/mars/jupiter/saturn/uranus/neptune/pluto/chiron/mean_node/true_node 扩展: mean_lilith/true_lilith (
Caelus.EXTRA_BODIES) 
...
Caelus.chartSignature(chart)
 → {elements:{},modalities:{},angularity:{},dominant:{element,modality,sign},ruler,hemispheres,quadrants,bodies} ctx.atoms→kind:"pattern"
 → T-square/Kite/Stellium/MysticRectangle/GrandTrine/GrandCross/Yod 
Caelus.detectPatterns(chart)
 → [{kind,bodies,apex?,orb}] 同上,直接取 ctx.atoms→kind:"reception"→ 互容(domicile/exaltation/triplicity+mixed) ctx.atoms→kind:"dispositor"→ 定位星链 ctx.atoms→kind:"dignity"
 → almuten/face/term/triplicity 
...
Caelus.declinationAspects(e,
Caelus.DEFAULT_BODIES,jd,1) → [{a,b,kind:"parallel"|"contraparallel"},...] 
...
Caelus.solarElongation(e,"mercury",jd) → 日距(度) engine.heliocentric("mars",jdUt) → {lon,lat,dist} (Engine实例, 不是
Caelus.) 
...
── 法达: 
Caelus.firdariaAt(e,natalJd,targetJd,lat,lon) → {day,major,sub} ⚠️ 必须传targetJd; 75年外返回{null,null} 
Caelus.firdaria(day,natalJd) → 完整周期表 ZR: 
Caelus.zrAt(e,natalJd,targetJd,lat,lon) → {lot,lot_sign,day,l1?,l2?,l3?,l4?} 主限: 
Caelus.primaryDirections(e,jd,lat,lon,bodies?,key?,maxYears?,yearLength?)
 → [{body,angle:"MC"|"IC"|"ASC"|"DSC",arc,years,jd}] 时间键 KEYS:{naibod:0.9856,ptolemy:1.0,brahe:0.986,cardan:0.985,simmonite:0.985} 世俗: 
Caelus.mundaneDirections(e,natalJd,lat,lonEast,bodies?,key?,maxYears?,yearLength?) → [{promissor,significator,arc,years,jd}] Placidus半弧, 行星到行星 太阳弧:
Caelus.solarArc(e,natalJd,targetJd,yearLength?,zodiac?) → 度数值 等价: 
...
Caelus.crossings(e,body,targetLon,jdStart,jdEnd,zodiac?,maxHits?) → [jd,...] ⚠️ 逆行体可穿3次同经度, 全部返回, 按时间排序 
── 恒星(2个) 
── e.starConjunctions(chart,{orb}) → [{body,star,orb},...] Caelus.starParans(e,jd,lat,stars,bodies?) → [{star,star_angle,body,body_angle,jd,gap_min},...] 
── 天文事件(5个) 
...
Caelus.houseOf(lon,cusps)/
Caelus.houseLord(ascSign,n) (ascSign=热带0-11) 
── 上下文增强 
...
── 调它只有两种情况: ① Caelus和NatalEngine对同一行星输出不同星座时，以它为准(仲裁) ② 问日食月食精确到秒的时刻时，用它拿秒级时间(Caelus返回类型+时间范围) 其余不调。CAELUS_VSOP87 != ASTRONOMY_VSOP87 (6弧秒算法差<400弧秒位置模糊, 调了等于没调)。 调用: var t=new Astronomy.MakeTime(new Date(Date.UTC(y,m-1,d,h,m))); Astronomy.EclipticLongitude(Astronomy.Body.Mercury,t)
 → 黄经 Sun用Astronomy.SunPosition(t).elon, Moon用new Astronomy.Ecliptic(Astronomy.GeoMoon(t)).elon Astronomy.SearchLunarEclipse(new Date(...)) / SearchGlobalSolarEclipse(...) Astronomy.SearchRiseSet(Astronomy.Body.Sun,new Astronomy.Observer(lat,lon,0),1,new Date(...),1) Astronomy.Seasons(2026) / Astronomy.MoonPhase(new Date(...)) 

── ⑤ HoroscopeJS（已被Caelus完全覆盖，不再推荐） 
──
 ⚠️ 日期参数是date不是day: {year,month,date,hour,minute} ╔══════════════════ 参数坑 ══════════════════╗ ║ vargaAt(e,jd,9) ← 数字,不是"D9" ║ ║ hasAspect({})(ctx) ← 柯里化,不是(chart) ║ ║ lots(e,jd,lat,lon) ← 不是hermeticLots ║ ║ firdariaAt 必须传 targetJd ║ ║ compositeLongitudes(e,jdA,jdB,bodies) ║ ║ dignities("sun",2) ← sign是0-11索引 ║ ║ almuten(84.13) ← 裸经度不是body名 ║ ║ outOfBounds(e,body,jd)← 不是(body,decl) ║ ╚═════════════════════════════════════════════╝ 其余 200+ 函数用 dir(Caelus) 自探索: 底层天文(sunApparent/nutation/precessEcliptic), 尊贵原子(dignityScore/faceRuler/termRuler/signRuler), 组合器(matchAll/matchAny), 特殊点(meanNode→弧度/57.2958转度/meanLilith/trueLilith/vertexEastPoint), lotFortune/lotSpirit/hermeticLots, 探测(houseCusp/angles/gmst/gast/normalizeHouseSystem)等。
...
NatalEngine(西洋+吠陀+人类图)
 → eval_javascript(library='natalengine-engine', code='NatalEngine.calculateAstrology("1990-06-15",12,8,39.9,116.4)')
 → {bigThree,summary,sun,moon,rising,midheaven,balance,planets,nodes,allAspects}

...
def _bazi(year, month, day, hour, gender=1):
    ...
    result = {
        ...
        "dayun":dayun_list,"start_age":yun.getStartYear(),"gender":gender,"lunar":str(l),
    }
    try:
...
def _yijing(method="time",seed=None,year=None,month=None,day=None,feature="all"):
    result={"system":"yijing","engine":"","_hint":"ichingshifa(Python)已返回起卦+解卦。JS双引擎对照(IchingShifa)/meihua_yi体用/taixuanshifa+TaixuanLib/jingjue。自探索:dir(ichingshifa)/Object.keys(IchingShifa)"}
    hex_values=None
    try:
        sys.path.insert(0,os.path.dirname(__file__))
        if method=="dayan":
            from ichingshifa import iching_dayan
            hex_data=iching_dayan(seed=seed)
            result["ichingshifa"]=str(hex_data)
        elif method=="time" and all([year,month,day]):
            from ichingshifa import iching_time
            hex_data=iching_time(year,month,day)
            result["ichingshifa"]=str(hex_data)
        elif method=="number":
            from ichingshifa import iching_number
            hex_data=iching_number(seed or 42)
            result["ichingshifa"]=str(hex_data)
        else:
            from ichingshifa import iching_time
            import datetime
            now=datetime.datetime.now()
            hex_data=iching_time(now.year,now.month,now.day)
            result["ichingshifa"]=str(hex_data)
    ...
    if feature=="all":
        ...
        if hex_values:
            js_arr=json.dumps(hex_values if isinstance(hex_values,list) else list(hex_values))
            result["iching_shifa_js"]=_js("iching-shifa-engine",f"JSON.stringify(IchingShifa.interpret({js_arr}))")
        else:
            result["iching_shifa_js"]=_js("iching-shifa-engine","JSON.stringify(IchingShifa.interpret([6,7,8,9]))")
